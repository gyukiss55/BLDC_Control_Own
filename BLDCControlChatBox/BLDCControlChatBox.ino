...
volatile int vol_ti_ix_rd = 0; // Timer index
volatile int vol_ti_ix_wr = 0; // Timer index
...
// Timer interrupt 10us period [web:16][web:13]
  ...
  MyTim->attachInterrupt(handleTimerInterrupt);
  MyTim->resume();
}
...
void handleTimerInterrupt() {
  ...
  vol_ti_ix_rd = (vol_ti_ix_rd + 1) % 32; // Increment and wrap index
  vol_ti_ts[vol_ti_ix_rd] = micros(); // Store the current time in microseconds
